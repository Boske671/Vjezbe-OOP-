...
private:
	int re, im;
public:
	Kompleksni(int re, int im) : re(re), im(im) { };

	friend ostream& operator << (ostream& izlaz, Kompleksni Z);
	Kompleksni& operator + (Kompleksni& k)
	{
		this->re = re + k.re;
		this->im = im + k.im;
		return *this;
	...
	Kompleksni operator * (Kompleksni& k)
	{
		return Kompleksni(re * k.re - im * k.im, re * k.im + k.re * im);
	}
	friend Kompleksni operator - (Kompleksni Z, double re);
	...
	Kompleksni operator ++(int) // postfiks ++
	{
		Kompleksni Pom = *this;
		++(*this);
		return Pom;
...
int main() {
	...
	// ne-članska operatorska funkcija - (double, Kompleksni)
	Kompleksni Razlika2 = 3 - Suma;
	cout << Razlika2 << endl << endl; // 0 - 1i


	Kompleksni proba(0, 0);
	cout << proba++ << endl;
	cout << ++proba << endl;
